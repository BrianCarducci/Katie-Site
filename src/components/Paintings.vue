<template>
  <div>
    ...
    <div class="text-2xl text-center pt-6 title">Paintings</div>
    <div class="grid pt-6 pb-8">
      <div class="max-w-sm rounded overflow-hidden shadow-lg mx-auto" v-for="url in imgUrls">
        <img
          class="mx-auto p-2 h-64 thumbnail"
          :src="url"
          v-on:click="expandImg"
          alt="Failed to load."
        />
      </div>
    </div>
    <div class="text-2xl text-center title">Studies</div>
    <div class="grid pt-6 pb-8">
      <div class="max-w-sm rounded overflow-hidden shadow-lg mx-auto" v-for="url in studyUrls">
        <img
          class="mx-auto p-2 h-64 thumbnail"
          :src="url"
          v-on:click="expandImg"
    ...
    paintingsRef
      ...
      .then(result => {
        result.items.forEach(imgRef => {
          imgRef.getDownloadURL().then(url => this.imgUrls.push(url));
        });
      })
      .catch(err => {
        console.log(err);
      });
    let studiesRef = firebase.storage().ref("Paintings/Studies");
    console.log("studiesRef: " + studiesRef);
    studiesRef
      .listAll()
      .then(result => {
        result.items.forEach(imgRef => {
          imgRef.getDownloadURL().then(url => this.studyUrls.push(url));
        });
      })
      .catch(err => {
        console.log(err);
